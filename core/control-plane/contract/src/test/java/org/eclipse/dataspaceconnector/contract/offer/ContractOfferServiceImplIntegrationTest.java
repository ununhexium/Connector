...
class ContractOfferServiceImplIntegrationTest {
    ...
    void shouldLimitResult_withHeterogenousChunks() {
        ...
        var query = ContractOfferQuery.builder().range(range).claimToken(ClaimToken.Builder.newInstance().build()).build();

        assertThat(contractOfferService.queryContractOffers(query, range)).hasSize(to - from);
    ...
    void should_return_offers_subset_when_across_multiple_contract_definitions(int from, int to) {
        ...
        var range = new Range(from, to);
        var query = ContractOfferQuery.builder().range(range).claimToken(ClaimToken.Builder.newInstance().build()).build();
        assertThat(contractOfferService.queryContractOffers(query, range)).hasSize(min(requestedRange, maximumRange));
    }
    ...
    void shouldLimitResult_insufficientAssets() {
        ...
        assertThat(contractOfferService.queryContractOffers(query, range)).hasSize(4);
        verify(agentService).createFor(isA(ClaimToken.class));
    ...
    void shouldLimitResult_pageOffsetLargerThanNumAssets() {
        ...
        assertThat(contractOfferService.queryContractOffers(query, range)).isEmpty();
        verify(agentService).createFor(isA(ClaimToken.class));
        verify(contractDefinitionService).definitionsFor(isA(ParticipantAgent.class));
